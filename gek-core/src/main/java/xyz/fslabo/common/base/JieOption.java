 ...
 * <pre>
 *     public void doSomething(Object arg, JieOption&lt;?, ?&gt;... options)
 * </pre>
 ...
 */
public interface JieOption<K, V> {
     ...
     */
    static <K, V> JieOption<K, V> of(K key, V value) {
     ...
     */
    static <K, V> JieOption<K, V>[] emptyOptions() {
     ...
     */
    @Nullable
    static <K, V> V get(K key, JieOption<?, ?>... options) {
        if (GekArray.isEmpty(options)) {
            return null;
        }
        for (JieOption<?, ?> option : options) {
     ...
     */
    static <K, V> V get(K key, V defaultValue, JieOption<?, ?>... options) {
        if (GekArray.isEmpty(options)) {
            return defaultValue;
        }
        for (JieOption<?, ?> option : options) {
     ...
     */
    static <K, V> V get(K key, JieOption<?, ?>[] options, Function<K, V> defaultValue) {
        if (GekArray.isEmpty(options)) {
     ...
     * @return option value of specified key in given options, or result of default value if not found
     */
    static <K, V> V get(K key, Iterable<? extends JieOption<?, ?>> options, Function<K, V> defaultValue) {
        if (GekArray.isEmpty(options)) {
            return defaultValue.apply(key);
        }
        for (JieOption<?, ?> option : options) {
