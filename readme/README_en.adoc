...
Boat is a set of core Java/Kotlin libraries (JDK 1.8+), written by kotlin (mostly) and java, widely used on Java/Kotlin projects within SrcLab, and also can be used by other organizations' Java/Kotlin projects.
...
* *boat-serialize*: serialization function, including json;
...
* *DefaultNonNull*/*DefaultNullable*: indicates all parameters, variables, fields or other types are non-null/null by default, usually used in package-info.java.
These annotations extend Nonnull of jsr305, and IDE such as IDEA can recognize them;
* *NonNull*/*Nullable*: indicates a parameter, variable, field or other type is non-null/null.
...
* Helper types: SimpleAccessor, CachingProductBuilder.
...
* BeanResolver: interface to resolver bean, actually Beans use a default BeanResolver to do its works;
...
Convert package provides type-conversion function, with Converts by default or Converter to custom.
...
Invoke package provides Invoker to call methods (for Java) and functions (for Kotlin).
...
Run package provides Runner and ScheduledRunner interfaces to execute tasks in threads or coroutines.
