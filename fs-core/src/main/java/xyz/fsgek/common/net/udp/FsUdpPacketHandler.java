 ...
 * {@link #onPacket(FsUdpHeader, FsUdpClient, Object)} of first handler. Returned object from first handler will be passed to next
 * handler as {@code packet}, and so on.
 * If null is returned, the handlers calling chain will be broken;
 * if an exception is thrown, {@link FsUdpServerHandler#onException(FsNetServerException)}
 * or {@link FsUdpServerHandler#onException(FsUdpHeader, FsUdpClient, Throwable, ByteBuffer)} will be called
