= image:../../logo.svg[Boat Serialize] `boat-other`: Boat Others -- link:../../README.md[Boat] 第三方支持和扩展库
:toc:
:toclevels: 3
:toc-title: 目录
:last-update-label!:
...
* `ProtobufBeanProvider`: 支持 protobuf 类型的 `BeanConvertHandler.NewBeanProvider` (boat-core);
