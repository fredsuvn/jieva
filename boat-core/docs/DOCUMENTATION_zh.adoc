...
* 字符串功能: `BFormat`, `BTemplate`, `BNamingCase`;
* 特殊字符支持: `BCtlChars`, `BEscChars`, `BCsiChars`, `BSgrChars`;
* 访问接口: `ObjectAccessor`, `MapAccessor`, `BAccessor`, `BMapAccessor`;
...
* `BConvertHandler`: `Converter` 用来转换的处理器;
...
* `ProxyMethod`: 代理的方法体;
* `ProxyClassGenerator`: 创建 `ProxyClass` 的工厂, 内置 `Reflection`, `spring-core` 和 `cglib` 实现;
...
* `BCollectType`, `BMapType`: 代表集合泛型类型;
* `BMultiMaps.kt`: `BMultiMaps.kt` 提供多值 `Map` 如 `BSetMap`, `MutableSetMap`, `BListMap` 何 `MutableListMap`;
