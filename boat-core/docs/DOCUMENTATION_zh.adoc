...
* <<Proxy>>: 提供 `ClassProxy` 来进行动态代理, 反射, `spring-core` 或 `cglib` 方式;
...
* 语法增强 (主要针对Java): `Let`, `Ref`, `Lazy`, `BLazyGetterCharSeq`;
* 字符串功能: `StringFormat`, `StringTemplate`, `BNamingCase`;
* 特殊字符支持: `BCtlChars`, `BEscChars`, `BCsiChars`, `BSgrChars`;
* 访问接口: `ObjectAccessor`, `MapAccessor`, `BAccessRef`, `BMapAccessor`;
...
Proxy包提供 `ClassProxy` 用带进行动态代理, 使用 `反射`, `spring-core` 或 `cglib` 方法:

* `ClassProxy`: 代理目标 `Class` 的核心接口;
* `ProxyMethod`: 代理的方法体;
* `ClassProxyFactory`: 创建 `ClassProxy` 的工厂, 内置 `Reflection`, `spring-core` 和 `cglib` 实现;
...
* `MultiMaps.kt`: `MultiMaps.kt` 提供多值 `Map` 如 `MutableSetMap`, `MutableSetMap`, `MutableListMap` 何 `MutableListMap`;
* `BCopyOnWriteMap`: 实现copy-on-write操作的Map;
