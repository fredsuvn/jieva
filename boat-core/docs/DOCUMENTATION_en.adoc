...
* <<Proxy>>: Provides `ClassProxy` to proxy a class in runtime dynamically, by `reflection`, `spring-core` or `cglib` ways;
...
* Lang syntax enhancement (mainly for Java): `Let`, `Ref`, `Lazy`, `BLazyGetterCharSeq`;
* String/CharSequence functions: `StringFormat`, `StringTemplate`, `BNamingCase`;
* Special character support: `CtlChars`, `EscChars`, `BCsiChars`, `SgrChars`;
* Accessor interfaces: `ObjectAccessor`, `MapAccessor`, `BAccessRef`, `BMapAccessor`;
...
* `RunWork`: Core interface represents a running thread;
* `Scheduler`: Core interface to schedule tasks;
* `ScheduleWork`: Core interface represents a scheduling task;
...
Proxy package provides `ClassProxy` to proxy a class in runtime dynamically, by `reflection`, `spring-core` or `cglib` ways:

* `ClassProxy`: Core interface to proxy target `Class`;
* `ProxyInvoker`: Core interface represents proxy method body;
* `ClassProxyProvider`: Factory to create `ClassProxy`, built-in `Reflection`, `spring-core` and `cglib` implementation;
...
* `MultiMaps.kt`: `MultiMaps.kt` provides multi-values `Map` such as `MutableSetMap`, `MutableSetMap`, `MutableListMap` and `MutableListMap`;
* `BCopyOnWriteMap`: Map for copy-on-write operation;
