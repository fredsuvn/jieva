...
* String/CharSequence functions: `BFormat`, `BTemplate`, `BNamingCase`;
...
* `BConvertHandler`: Handler to process converting for `Converter`;
...
* `BCollectType`, `BMapType`: Meta type interfaces for generic Collection types;
* `BMultiMaps.kt`: `BMultiMaps.kt` provides multi-values `Map` such as `BSetMap`, `MutableSetMap`, `BListMap` and `MutableListMap`;
