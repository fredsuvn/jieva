...
public class CodecTest {
    ...
    @Test
    public void testRsa() {
        RsaCipher rsaCipher = Coding.rsaCipher();
        ...
        byte[] encrypt2 = Coding.forData(data).encryptRsa(rsaKeyPair.publicKey()).doFinal();
        byte[] decrypt2 = Coding.forData(encrypt2).decryptRsa(rsaKeyPair.privateKey()).doFinal();
        Assert.assertEquals(decrypt1, dataBytes);
        Assert.assertEquals(decrypt2, dataBytes);

        String decryptData = Coding.forData(encrypt1).decryptRsa(rsaKeyPair.privateKeyBytes()).doFinalToString();
        Assert.assertEquals(decryptData, data);

        //Test empty string
        String emptyData = "";
        byte[] encryptEmpty = Coding.forData(emptyData).encryptRsa(rsaKeyPair.publicKey()).doFinal();
        String decryptEmptyString = Coding.forData(encryptEmpty).decryptRsa(rsaKeyPair.privateKey()).doFinalToString();
        Assert.assertEquals(decryptEmptyString, emptyData);
    }

    @Test
    public void testSm2() {
        Sm2Cipher sm2Cipher = Coding.sm2Cipher();
        ...
        byte[] encrypt2 = Coding.forData(data).encryptSm2(sm2KeyPair.publicKey()).doFinal();
        byte[] decrypt2 = Coding.forData(encrypt2).decryptSm2(sm2KeyPair.privateKey()).doFinal();
        Assert.assertEquals(decrypt1, dataBytes);
        Assert.assertEquals(decrypt2, dataBytes);

        String decryptData = Coding.forData(encrypt1).decryptSm2(sm2KeyPair.privateKeyBytes()).doFinalToString();
        Assert.assertEquals(decryptData, data);

        //Test empty string
        String emptyData = "";
        byte[] encryptEmpty = Coding.forData(emptyData).encryptSm2(sm2KeyPair.publicKey()).doFinal();
        String decryptEmptyString = Coding.forData(encryptEmpty).decryptSm2(sm2KeyPair.privateKey()).doFinalToString();
        Assert.assertEquals(decryptEmptyString, emptyData);
    }
    ...
    public void testAes() {
        ...
        String data = random(512);
        byte[] dataBytes = Chars.toBytes(data);
        byte[] encrypt = Coding.aesCipher().encrypt(secretKey, dataBytes);
        byte[] decrypt = Coding.aesCipher().decrypt(secretKey, encrypt);
        Assert.assertEquals(decrypt, dataBytes);

        String decryptData = Coding.forData(encrypt).decryptAes(secretKey.getEncoded()).doFinalToString();
        Assert.assertEquals(decryptData, data);

        //Test empty string
        String emptyData = "";
        byte[] encryptEmpty = Coding.forData(emptyData).encryptAes(secretKey).doFinal();
        String decryptEmptyString = Coding.forData(encryptEmpty).decryptAes(secretKey).doFinalToString();
        Assert.assertEquals(decryptEmptyString, emptyData);
    }
    ...
    public void testCodec() {
        ...
        //加密
        byte[] encrypt = Coding.aesCipher().encrypt(aesKey.getEncoded(), Chars.toBytes(data));
        encrypt = rsaCipher.encrypt(rsaKeyPair.publicKey(), encrypt);
        encrypt = sm2Cipher.encrypt(sm2KeyPair.publicKey(), encrypt);
        encrypt = Coding.encodeBase64(encrypt);

        //解密
        String origin = Coding.forData(encrypt)
            .decodeBase64()
    ...
    public void testPassword() {
        ...
        logger.log("password: {}, aes: {}",
            password, Coding.aesCipher().encryptToBase64String(key, password));
    }
}
