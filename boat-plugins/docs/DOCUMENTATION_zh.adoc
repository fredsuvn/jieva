...
boat-protobuf 提供为 `bean`, `convert` 何 `data` 领域的针对 protobuf 类型的处理器:

* `ProtobufBeanResolveHandler`: 支持 protobuf 类型的bean解析器, 从 `BeanResolveHandler` (boat-core) 扩展;
* `ProtobufBeanCreator`: 支持 protobuf 类型的 `BeanConvertHandler.NewBeanProvider` (boat-core);
...
* ProtobufJsons.PROTOBUF_OBJECT_MAPPER: ``Jackson``的 `ObjectMapper` (boat-data), 支持 protobuf 类型;
* ProtobufJsons.PROTOBUF_JSON_SERIALIZER: `DataParser` (boat-data), 支持 protobuf 类型.
